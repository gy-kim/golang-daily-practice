...
const (
	// default person id (required on error)
	defaultPersonID = 0
)
...
// Save will save the supplied person and return the ID of the newly creatd person or an error.
...
func LoadAll() ([]*Person, error) {
	...
	for rows.Next() {
		...
		if err != nil {
			logging.L.Error("failed to convert query resut. err: %s", err)
			return nil, err
		}
...
// Load will  attemp to load and return a person.
