...
func (c *converter) ConvertValue(v interface{}) (driver.Value, error) {
	return nil, errors.New("converter disable")
}
...
func TestShouldOpenConnectionIssue15(t *testing.T) {
	db, mock, err := New()
	if err != nil {
		t.Errorf("expected on error, but got: %s", err)
	}
	if len(pool.conns) != 1 {
		t.Errorf("expected 1 connection is pool, but there is: %d", len(pool.conns))
	}
	...
	defer func() {
		if smock.opened != 0 {
			t.Errorf("expected no connections on mok to be opened, but there is: %d", smock.opened)
		}
	}()
	...
	if err = db.Close(); err != nil {
		t.Errorf("unexpected no error on close, but got: %s", err)
	}
