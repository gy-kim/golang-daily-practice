...
import (
	...
	"github.com/gy-kim/golang-daily-practice/2019/05-May/11-20/13/hands-on-dependency-injection-in-go/ch06/03_applying/01/data"
	"github.com/gy-kim/golang-daily-practice/2019/05-May/11-20/13/hands-on-dependency-injection-in-go/ch06/03_applying/01/register"
)

// RegisterHandler is the HTTP handler for the "Register" endpoint
// In this simplified example we are assuming all possible errors are user errors and returning "bad requeest" HTTP 400.
...
func (h *RegisterHandler) ServeHTTP(response http.ResponseWriter, request *http.Request) {
	...
	if err != nil {
		// not need to log here as we can expect other layer to do so
		response.WriteHeader(http.StatusBadRequest)
		return
	}
