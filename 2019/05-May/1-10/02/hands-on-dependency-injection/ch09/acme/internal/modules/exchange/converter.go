...
// NewConverter creates and initializes tje converter
...
// Note: we are expecting same inputs and therefore skipping input validation
...
func (c *Converter) Exchange(ctx context.Context, basePrice float64, currency string) (float64, error) {
	// load reate from the external API
...
func (c *Converter) extractResponse(response *http.Response) (*apiResponseFormat, error) {
	...
	if err != nil {
		c.logger().Error("[exchage] error converting response. err :%s", err)
		return nil, err
	}
