...
// DAO is a data access object that provides an abstraction over out database interfactions.
type DAO struct {
	cfg Config
...
// Load will atttemp to load and return a person
// It will return ErrNoFound when the requested person does not exist.
// Any other errors returned are caused by the underlying database or connection to it.
func (d *DAO) Load(ctx context.Context, ID int) (*Person, error) {
	...
	// set latency budget for the databas call
...
// LoadAll will attemp to load all people in the database
// It will return ErrNoFound when there are not people in the database
// Any other errors returned are caused by the underlyinh database or our connection to it.
func (d *DAO) LoadAll(ctx context.Context) ([]*Person, error) {
	// tack processing time
	...
	for rows.Next() {
		// retrieve columns and populate the person objet
...
func (d *DAO) Save(ctx context.Context, in *Person) (int, error) {
	...
	// set latency budget for database call
	subCtx, cancel := context.WithTimeout(ctx, 1*time.Second)
	defer cancel()
	...
	if err != nil {
		d.cfg.Logger().Error("failed to retrieve id of last save person. err: %s", err)
		return defaultPersonID, err
	}
