...
func TestLoadAll_tableDrivenTest(t *testing.T) {
	...
	for _, s := range scenarios {
		...
		// validate results
		assert.Equal(t, s.expectedResults, results, s.desc)
		assert.Equal(t, s.expectError, err != nil, s.desc)
		assert.NoError(t, dbMock.ExpectationsWereMet())
...
func TestLoad_tableDrivenTest(t *testing.T) {
	...
	for _, s := range scenarios {
		...
		// validate results
		assert.Equal(t, s.expectedResult, result, s.desc)
		assert.Equal(t, s.expectError, err != nil, s.desc)
		assert.NoError(t, dbMock.ExpectationsWereMet())

		// restore originla DB (after test)
		db = &original
		testDb.Close()
	}
}
