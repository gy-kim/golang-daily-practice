...
func sleepRandom(fromFunction string, ch chan int) {
	...
	// Write on the channel if it passed in
	if ch != nil {
		ch <- sleeptime
	}
}
...
func sleepRandomContext(ctx context.Context, ch chan bool) {
	...
	case <-ctx.Done():
		...
		// Signal all the goroutinew that should stop work (use channels)
...
func doWorkContext(ctx context.Context) {
	...
	// Timeout in 150ms
	// All the contexts derived from this will return in 150 ms
	...
	// As we do use the return value sent on channel
	ch := make(chan bool)
